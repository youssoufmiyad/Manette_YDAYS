...
NULL,
  ...
  NULL,
#else
		USBD_HID_GetHSCfgDesc, USBD_HID_GetFSCfgDesc,
...
static uint8_t USBD_HID_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx) {
	...
	hhid = (USBD_HID_HandleTypeDef*) USBD_malloc (
			sizeof(USBD_HID_HandleTypeDef));
 ...
 */
static uint8_t* USBD_HID_GetFSCfgDesc(uint16_t *length) {
	USBD_EpDescTypeDef *pEpDesc = USBD_GetEpDesc(USBD_HID_CfgDesc,
			HID_EPIN_ADDR);
